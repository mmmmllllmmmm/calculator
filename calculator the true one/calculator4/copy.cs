    ...
    public partial class Form1 : Form
    {
        ...
        private void priority(string received)
        {
            ...
            else
            {
                ...
                {     
                    if (operations.Contains("("))////////////////////////////////////delete this
                    {
        ...
        private void button12_Click(object sender, EventArgs e)
            ...
            if(pr.Text=="."&&numhold==""&&textBox1.Text!="0")
            {
                numhold = "0.";
            ...
            if (textBox1.Text.Length > 0 && textBox1.Text != "0")
            {
                if (check == "+" || check == "-" || check == "*" || check == "/"||check=="%")
                {
                    operations.Pop();
                        ...
                        if (previous == ')')//في حال كان اخر شي في المصفوفه هو غلقه القوس يافاااااااااااااااااااضل+ 
                        {
                    ...
                    else if (check == "(")// في خطا عند الحذف الكبير
                    {
                        ...
                        if (previous == ')')
                        {
                            if (posfix[i - 2] == "+" || posfix[i - 2] == "*" || posfix[i - 2] == "-" || posfix[i - 2] == "/"||posfix[i-2]=="%" || posfix[i - 2] != "%" || posfix[i - 2] == ")" || posfix[i - 2] == "(")
                            {
                                if(posfix[i - 3] != "+" || posfix[i - 3] != "*" || posfix[i - 3] != "-" || posfix[i - 3] != "/"||posfix[i-3]!="%" || posfix[i - 3] != "%" || posfix[i - 3] != ")" || posfix[i - 3] != "(")
                                if (double.Parse(posfix[i - 3]) < 0)
                                {
                                    sign = true;
                                }
                            }
                            ...
                            {
                                if (posfix[i - 2] != "+" || posfix[i - 2] != "*" || posfix[i - 2] != "-" || posfix[i - 2] != "/" || posfix[i - 2] != "%"||posfix[i-2]!=")"|| posfix[i - 2] != "(")
                                    if (double.Parse(posfix[i - 2]) < 0)
                                {
                                    sign = true;
                                }
                            }
        ...
        private void button19_Click(object sender, EventArgs e)
        {
                ...
                numhold= idit.Pop().ToString();
               ///////////// if(int.Parse(numhold)>=0)
               if(double.Parse(numhold)>=0)
                {
                textBox1.Text = numhold; 
                }
                else
        ...
        private void button26_Click_1(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please insert the number before the " + math.Text);
            }
        }
